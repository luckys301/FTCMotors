...
public class RightSpline extends SequentialCommandGroup{
    public RightSpline(MecDrivetrainSubsystem mecDrivetrainSubsystem, Slide slide, Pivot pivot, Claw claw, TurnServo turnServo){
        /*Turn is Counterclockwise*/
        addCommands(
                new ParallelCommandGroup(
                        new ArmHighFrontCommand(slide, pivot, claw, turnServo, true),
                        new SplineCommand(mecDrivetrainSubsystem, new Vector2d(58.8, 8.4), Math.toRadians(32))    //Cycle
                ),
                new AutoDropConeCommand(claw, slide, pivot, true),
                new ParallelCommandGroup(
                        new ArmCone5BackCommand(slide, claw, pivot, turnServo),
                        new SplineCommand(mecDrivetrainSubsystem, new Vector2d(52.5, -23), Math.toRadians(268), true)   //Load
                ),
                ...
                new ParallelCommandGroup(
                        new ArmHighFrontCommand(slide, pivot, claw, turnServo, true),
                        new SplineCommand(mecDrivetrainSubsystem, new Vector2d(58.8, 8.4), Math.toRadians(32), PoseStorage.cycle)    //Cycle
                ),
                new AutoDropConeCommand(claw, slide, pivot, true),
                new ParallelCommandGroup(
                        new ArmCone5BackCommand(slide, claw, pivot, turnServo),
                        new SplineCommand(mecDrivetrainSubsystem, new Vector2d(52.5, -23), Math.toRadians(268), PoseStorage.load, true)   //Load
                ),
                new AutoPickConeCommand(slide, claw)
...
//                new ParallelCommandGroup(
//                        new SlideHighFCommand(slide, arm, claw, true),
//                        new SplineCommand(mecDrivetrainSubsystem, new Vector2d(58, 8.5), Math.toRadians(23), PoseStorage.cycle)    //Cycle
//                ),
//                new DropAutoConeCommand(claw, slide, arm, true),
//
//                new SplineCommand(mecDrivetrainSubsystem, new Vector2d(58,8.5), Math.toRadians(0))
//                new TurnToCommand(mecDrivetrainSubsystem, 0) - Can't Use without IMU
        );
    }
}