...
public class LeftSpline extends SequentialCommandGroup{
    public LeftSpline(MecDrivetrainSubsystem mecDrivetrainSubsystem, Slide slide, Pivot pivot, Claw claw, TurnServo turnServo){
        /*Turn is Counterclockwise*/
        addCommands(
                new ParallelCommandGroup(
                        new ArmHighFrontCommand(slide, pivot, claw, turnServo, true),
                        new SplineCommand(mecDrivetrainSubsystem, new Vector2d(LeftSplineValues.AToHighOne.x1,LeftSplineValues.AToHighOne.y1), Math.toRadians(LeftSplineValues.AToHighOne.heading1))    //Cycle
                ),
                new AutoDropConeCommand(claw, slide, pivot, true),
                new ParallelCommandGroup(
                        new ArmCone5BackCommand(slide, claw, pivot, turnServo),
                        new SlowSplineCommand(mecDrivetrainSubsystem, new Vector2d(LeftSplineValues.BToConeOne.x, LeftSplineValues.BToConeOne.y2), Math.toRadians(LeftSplineValues.BToConeOne.heading2), true)   //Load
                ),
                new AutoPickConeCommand(slide, claw),
//        new IntakeAutoCommand(mecDrivetrainSubsystem, slide, claw, sensorColor, true),


                new ParallelCommandGroup(
                        new ArmMidFrontCommand(slide, pivot, claw, turnServo,true),
                        new SplineCommand(mecDrivetrainSubsystem, new Vector2d(LeftSplineValues.BToMid.x3,LeftSplineValues.BToMid.y3), Math.toRadians(LeftSplineValues.BToMid.heading3))   //Cycle
                ),
                new WaitCommand(400),
                new AutoDropConeCommand(claw, slide, pivot, true),
                new ParallelCommandGroup(
                        new ArmCone4BackCommand(slide, claw, pivot, turnServo),
                        new SlowSplineCommand(mecDrivetrainSubsystem, new Vector2d(LeftSplineValues.CToConeOne.x2,LeftSplineValues.CToConeOne.y2), Math.toRadians(LeftSplineValues.CToConeOne.heading2), true)   //Load
                ),
                new AutoPickConeCommand(slide, claw),
//        new IntakeAutoCommand(mecDrivetrainSubsystem, slide, claw, sensorColor, true),
                ...
                new ParallelCommandGroup(
                        new ArmMidFrontCommand(slide, pivot, claw, turnServo,true),
                        new SplineCommand(mecDrivetrainSubsystem, new Vector2d(LeftSplineValues.CToMid.x3,LeftSplineValues.CToMid.y3), Math.toRadians(LeftSplineValues.CToMid.heading3))   //Cycle
                ),
                new WaitCommand(400),
                new AutoDropConeCommand(claw, slide, pivot, true),
                new ParallelCommandGroup(
                        new SlideResetUpAutonCommand(slide, pivot, claw, turnServo),
                        new SlowSplineCommand(mecDrivetrainSubsystem, new Vector2d(LeftSplineValues.DToConeOne.x2, LeftSplineValues.DToConeOne.y2), Math.toRadians(LeftSplineValues.DToConeOne.heading2), PoseStorage.cycle, true)   //Load
                )
